...
class LoadingScreen(Stage):
    def __init__(self, exit_stage=None, parent_stage=None):
    ...
    def cleanup(self):
        ...
        base.taskMgr.remove("LoadGameResourceTask")
        

    def transition_to_next_stage(self, exit_stage=None):
        """Proceed with loading the next stage."""
        print(self.exit_stage)
        if hasattr(base, "lvl"):
            base.lvl = exit_stage
        base.flow.transition(base.lvl)
        return
    ...
    def load_game_resources_task(self, task):
      ...
      # End the task to proceed to the next stage
            # Simulate loading tasks here
        taskMgr.doMethodLater(2.0, self.finish_loading, 'finish_loading_task')
        return task.done

    def finish_loading(self, task):
        # Transition to the parent stage when loading is done
        self.transition_to_next_stage("worldcage")
