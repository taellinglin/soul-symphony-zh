...
class room00(Stage):
    ...
    def actionA(self):
        ...
        if result.getNumContacts() > 0:
            ...
            if self.double_jump == 0:
                self.double_jump = 1
                print("Off ground, can Double Jump!")
            if self.double_jump == 1:
                self.player.ballNP.node().applyCentralImpulse(Vec3(0,0,128+32))
                print("Double Jump!")
                self.double_jump = 2
            elif self.double_jump == 2:
                print("Can only double jump!")
    ...
    def update(self, task):
        ...
        if result.getNumContacts() > 0:
                ...
                if contact.getNode1() == self.level.floorNP.node():
                    if self.double_jump != 0:
                        print("Landed on ground, can now jump and double jump!")
                    self.double_jump = 0
                    
        if(self.player.ballNP.getPos().z <= self.level.floor.getPos()-300):
