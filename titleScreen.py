...
from panda3d.core import TransparencyAttrib, CardMaker, NodePath, Vec4, Vec3, Point3, LVector3
...
class TitleScreen(Stage):
    ...
    def enter(self, data):
        ...
        base.bgm.playMusic('灵林 - Title', True)
    ...
    def create_logo_wave(self):
        ...
        def update_wave(task):
            ...
            scale_factor = wave_scale_min[0] + (wave_scale_max[0] - wave_scale_min[0]) * 0.5 * (1 + math.sin(2 * math.pi * frequency * current_time))
            y_offset = y_offset_amplitude * math.sin(2 * math.pi * frequency * current_time)

            # Update the object's scale and position
            self.imageObject.setScale(scale_factor, 1, 0.65)  # Assuming you want to keep y constant
    ...
    def star_and_logo(self):
        ...
        self.imageObject = OnscreenImage(image='graphics/SoulSymphonyLogoCN.png', pos=(-0.0, -0.0, 0.0), scale=(1, 1, 1))
        ...
        self.star_color_cycle = Sequence(
            ...
            LerpColorInterval(self.imageObject2, 0.04, Vec4(1, 0, 0, 0.15), startColor=Vec4(1, 0.5, 0, 0), blendType='easeInOut'),  # Red to Orange
            LerpColorInterval(self.imageObject2, 0.05, Vec4(1, 1, 0, 0.15), startColor=Vec4(1, 0, 0, 0), blendType='easeInOut'),    # Orange to Yellow
            LerpColorInterval(self.imageObject2, 0.06, Vec4(0, 1, 0, 0.15), startColor=Vec4(1, 1, 0, 0), blendType='easeInOut'),    # Yellow to Green
            LerpColorInterval(self.imageObject2, 0.07, Vec4(0, 0, 1, 0.15), startColor=Vec4(0, 1, 0, 0), blendType='easeInOut'),    # Green to Blue
            LerpColorInterval(self.imageObject2, 0.08, Vec4(0.29, 0, 0.51, 0.15), startColor=Vec4(0, 0, 1, 0), blendType='easeInOut'),  # Blue to Indigo
            LerpColorInterval(self.imageObject2, 0.09, Vec4(0.56, 0, 1, 0.15), startColor=Vec4(0.29, 0, 0.51, 0), blendType='easeInOut'),  # Indigo to Violet
            LerpColorInterval(self.imageObject2, 0.1, Vec4(1, 0, 0, 0.15), startColor=Vec4(0.56, 0, 1, 0), blendType='easeInOut')  # Violet to Red (loop)
        ...
        self.star_color_cycle2 = Sequence(
            ...
            LerpColorInterval(self.imageObject3, 0.1, Vec4(0.56, 0, 1, 0.15), startColor=Vec4(1, 0, 0, 0), blendType='easeInOut'),  # Red to Violet
            LerpColorInterval(self.imageObject3, 0.09, Vec4(0.29, 0, 0.51, 0.15), startColor=Vec4(0.56, 0, 1, 0), blendType='easeInOut'),  # Violet to Indigo
            LerpColorInterval(self.imageObject3, 0.08, Vec4(0, 0, 1, 0.15), startColor=Vec4(0.29, 0, 0.51, 0), blendType='easeInOut'),  # Indigo to Blue
            LerpColorInterval(self.imageObject3, 0.07, Vec4(0, 1, 0, 0.15), startColor=Vec4(0, 0, 1, 0), blendType='easeInOut'),    # Blue to Green
            LerpColorInterval(self.imageObject3, 0.06, Vec4(1, 1, 0, 0.15), startColor=Vec4(0, 1, 0, 0), blendType='easeInOut'),    # Green to Yellow
            LerpColorInterval(self.imageObject3, 0.05, Vec4(1, 0.5, 0, 0.15), startColor=Vec4(1, 1, 0, 0), blendType='easeInOut'),  # Yellow to Orange
            LerpColorInterval(self.imageObject3, 0.04, Vec4(1, 0, 0, 0.15), startColor=Vec4(1, 0.5, 0, 0), blendType='easeInOut')   # Orange to Red (loop)
    ...
    def load_textures(self):
        ...
        for filename in os.listdir(self.textures_path):
            if filename.endswith(('.png', '.jpg', '.jpeg')):  # Adjust for texture formats
    ...
    def set_star_texture(self, texture_index):
        ...
        if self.textures:
            ...
            self.star_placeholder.setTexture(texture_stage, self.textures[texture_index])
