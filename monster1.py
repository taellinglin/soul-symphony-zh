from panda3d.core import NodePath, Vec3, Vec4, GeomNode, CollisionNode, CollisionSphere, CollisionBox, CollisionRay, GeomVertexData, GeomVertexFormat, GeomVertexWriter, Geom, GeomTriangles, BitMask32, CollisionHandlerEvent, CollisionTraverser
...
class ProceduralMonster(NodePath):
    ...
    def handle_collision_in(self, entry):
        print(f"Collision detected! From: {entry.get_from_node_path()} To: {entry.get_into_node_path()}")

    def handle_collision_out(self, entry):
        print(f"Collision ended! From: {entry.get_from_node_path()} To: {entry.get_into_node_path()}")

    def generate_monster(self):
        
        """Generates a flower creature with petal-like body parts."""
        num_parts = random.randint(5, 8)  # Increased number of parts for more complexity
        colors = [
            Vec4(1, 0, 0, 1), Vec4(1, 0.5, 0, 1), Vec4(1, 1, 0, 1),
            Vec4(0, 1, 0, 1), Vec4(0, 0, 1, 1), Vec4(0.5, 0, 0.5, 1)
        ]
        ...
        body_collision.set_into_collide_mask(BitMask32.bit(0))  # Ignore other collisions
        ...
        for i in range(num_petals):
            ...
            petal_position = Vec3(math.cos(math.radians(angle)) * 1.0, math.sin(math.radians(angle)) * 1.0, 0)
            ...
            petal_collision.set_from_collide_mask(BitMask32.bit(1))  # Player collision layer
            petal_collision.set_into_collide_mask(BitMask32.bit(0))  # Ignore other collisions
            petal.attach_new_node(petal_collision)

            # Accept the collision event through the main event handler (ShowBase object)
            if self.event_handler:
                self.event_handler.accept("petal_{}-into".format(i), self.on_petal_collision)
    ...
    def create_sword(self):
        ...
        weapon_swinger.sword.set_color(Vec4(0.7, 0.7, 0.7, 1))  # Set the sword's color to grey

        # Position the sword in front of the monster (adjust based on your model)
        weapon_swinger.sword.set_pos(0.5, 0, 0)  # Example: positioned at (0.5, 0, 0) relative to the monster
        ...
        taskMgr.add(self.swing_weapon_task, "SwingWeaponTask", extraArgs=[weapon_swinger], interval=2)
    ...
    def start_attack(self, weapon_swinger):
        ...
        weapon_swinger.sword.set_h(weapon_swinger.sword.get_h() + 45)  # Rotate the sword for swinging effect
    ...
    def stop_attack(self, weapon_swinger):
        ...
        weapon_swinger.sword.set_h(weapon_swinger.sword.get_h() - 45)  # Reset the sword rotation to neutral
    ...
    def add_collision(self, part, part_type, size):
        ...
        collision_node.set_from_collide_mask(BitMask32.bit(1))  # This layer will collide with the player
        collision_node.set_into_collide_mask(BitMask32.all_off())  # Don't interact with other objects

        # Accept collision events if necessary
        if self.event_handler:
            self.event_handler.accept(f"{part.get_name()}-into", self.on_petal_collision)
    ...
    def create_physics_cage(self):
        ...
        min_x = min(petal.get_pos().x for petal in self.body_parts) - 1.0  # Adjust for padding
        ...
        cage_collision_node.add_solid(CollisionBox(Vec3((min_x + max_x) / 2, (min_y + max_y) / 2, (min_z + max_z) / 2), 
                                                max_x - min_x, max_y - min_y, max_z - min_z))
        cage_collision_node.set_from_collide_mask(BitMask32.bit(1))  # Allow player collisions
        cage_collision_node.set_into_collide_mask(BitMask32.bit(0))  # Ignore other collisions
    ...
    def create_sphere(self, radius):
        ...
        format = GeomVertexFormat.get_v3n3c4()  # Vertex format with position, normal, and color
        ...
        for vert in vertices:
            ...
            normal.add_data3f(*vert)  # Normals are the same as the vertex positions for spheres
